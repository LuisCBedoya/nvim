...
return {
  ...
  lsp_info = {
    function()
      ...
      -- LSP servers (excluyendo null-ls y copilot)
      ...
      -- FORMATTERS registrados
      local supported_formatters = formatter.list_registered(buf_ft) or {}
      for _, f in ipairs(supported_formatters) do
        if vim.fn.executable(f) == 1 then
          table.insert(formatters, ' ' .. f)
        end
      end

      -- conform.nvim fallback
      if #formatters == 0 then
        local status, conform = pcall(require, 'conform')
        if status then
          local conform_formatters = conform.list_formatters_for_buffer() or {}
          for _, f in ipairs(conform_formatters) do
            if vim.fn.executable(f) == 1 then
              table.insert(formatters, ' ' .. f)
            end
          end
      ...
      -- LINTERS registrados
      local supported_linters = linter.linter_list_registered(buf_ft) or {}
      for _, l in ipairs(supported_linters) do
      ...
      local function add_if_not_empty(items, output_table)
        if #items > 0 then
          table.insert(output_table, table.concat(items, ', '))
        end
      end

      add_if_not_empty(servers, output)
      add_if_not_empty(formatters, output)
      add_if_not_empty(linters, output)
